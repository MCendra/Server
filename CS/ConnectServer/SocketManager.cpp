...
constexpr char INIT_CREATE_SHUTDOWN_EVENT_ERROR_MSG[] = "[CS] CSocketManager::Init() - Error al crear evento de shutdown: %d";
constexpr char INIT_SUCCESS_MSG[] = "[CS] CSocketManager::Init() - Servidor iniciado en el puerto [%d]";
...
// Envia datos al cliente especificado, gestionando un buffer secundario
// ("side buffer") para no perder informacion si ya hay un WSASend en
// curso para ese cliente.
//
// Flujo:
//   - Valida indice/estado del cliente y tamaño del mensaje.
...
void CSocketManager::OnRecv(int index, DWORD IoSize, IO_RECV_CONTEXT* lpIoContext)
{
	...
	if (WSARecv(lpClientManager->m_socket, &lpIoContext->wsabuf, 1, &RecvSize, &Flags, &lpIoContext->overlapped, 0) == SOCKET_ERROR)
	{
		if (WSAGetLastError() != WSA_IO_PENDING)
		{
			Log.ToDisp(LOG_RED, "[SocketManager] WSARecv() fallo con el error: %d", WSAGetLastError());
			this->Disconnect(index);
			return;
		}
	}
...
//
// Flujo:
//   - IoSize == 0 -> error de envio: se desconecta al cliente.
...
void CSocketManager::OnSend(int index, DWORD IoSize, IO_SEND_CONTEXT* lpIoContext)
{
	...
	if (lpIoContext->IoMainBuffer.size >= lpIoContext->IoSize)
	{
		...
		{
			// Nada mas por enviar: liberamos el "lock" de envio.
...
int CALLBACK CSocketManager::ServerAcceptCondition(IN LPWSABUF lpCallerId, IN LPWSABUF lpCallerData, IN OUT LPQOS lpSQOS, IN OUT LPQOS lpGQOS, IN LPWSABUF lpCalleeId, OUT LPWSABUF lpCalleeData, OUT GROUP FAR* g, DWORD_PTR dwCallbackData)
{
	...
	if (gIpManager.CheckIpAddress(ipStr) == 0)
	{
		return CF_REJECT;
	}
	else
...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager)
{
	...
	while (true)
	{
		...
		SOCKET socket = WSAAccept(lpSocketManager->m_listen,
			(sockaddr*)&SocketAddr,
			&SocketAddrSize,
			(LPCONDITIONPROC)&lpSocketManager->ServerAcceptCondition,
			(DWORD_PTR)lpSocketManager);
		...
		if (socket == INVALID_SOCKET)
		{
			...
			{
				Log.ToDisp(LOG_RED, "[SocketManager] WSAAccept() fallo con error: %d", Error);
			}
			continue;
		}
...

		{
			...
			// (tercer parametro) sera el indice del cliente, y se recibira
			// en GetQueuedCompletionStatus para identificar a que cliente
			...
			lpClientManager = &gClientManager[index];

			lpClientManager->AddClient(
				index,
				ipAddress,
				socket);
		}
		...
		if (WSARecv(socket,
			&lpClientManager->m_IoRecvContext->wsabuf,
			1,
			&RecvSize,
			&Flags,
			&lpClientManager->m_IoRecvContext->overlapped,
			0) == SOCKET_ERROR)
		{
			int Error = WSAGetLastError();
...
DWORD WINAPI CSocketManager::ServerQueueThread(CSocketManager* lpSocketManager)
{
	...
	while (true)
	{
		...
		{
			Log.ToDisp(LOG_RED, "[SocketManager] WaitForMultipleObjects() failed with error: %d", GetLastError());
			break;
		}
