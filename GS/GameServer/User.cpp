...
#include "FakeOnline.h" //FakeOnline_EMU
...
//	lpObj->AttackCustomOfflineMoneyDelay = 0;
	...
	lpObj->UserSkinPick = 0;
#if(CHONPHEDOILAP)
	lpObj->ChonPheHanhTau = 0; //1 Dao Tac, 2 Bao Tieu
#endif
#if(B_HON_HOAN)
...
short gObjDel(int aIndex)
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		if(lpObj->Account[0] != 0){LogAddConnect(LOG_BLUE,"[Obj] [%d] Out Account:(%s)",aIndex,lpObj->Account);}

		LogAddConnect(LOG_BLUE, "[Obj] [%d] Ip Out Game (%s)", aIndex, lpObj->IpAddr);
...
bool gObjFixInventoryPointer(int aIndex)
{
	...
	{
#if(EVENT_END_LESS)
		if (lpObj->Transaction == 1 || lpObj->CB_Status == 1|| lpObj->CB_Status1 == 1)
#else
...
#if(EVENT_END_LESS)
	if (gBEventEndLess.UserDieProc(lpObj, lpTarget))
	{
		return;
	}
#endif
...
#if(CHONPHEDOILAP)
	if (lpObj->ChonPheHanhTau != lpTarget->ChonPheHanhTau && gServerInfo.MoChucNangChonPhe)
	{
		return;
	}
#endif
...
void gObjSecondProc()
{
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if (lpObj->Connected > OBJECT_LOGGED &&	lpObj->Type == OBJECT_USER)
		{
			...
			//=== Check Map Phe Win
			if (gCTCMini.CheckMapWin(lpObj->Index,lpObj->Map))
			{
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Map Chỉ Dành Cho Phe Chiến Thắng Công Thành !");
				gObjMoveGate(lpObj->Index, 17); //move ve lorencia
			}
...
#if(CHONPHEDOILAP)
	if (lpObj->ChonPheHanhTau != gObj[aTargetIndex].ChonPheHanhTau && gServerInfo.MoChucNangChonPhe)
	{
		return;
	}
#endif
