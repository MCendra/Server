...
void CDuel::CheckDuelScore(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if (gServerInfo.m_TienTrinhQuyetDau == 2)
	{
		if (lpObj->DuelScore >= gServerInfo.m_SoTranChamChien) // 
		{
			...
			/*if(lpInfo != gServerInfo.m_SoTranChamChien){
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,"Co 1 thang da thoat khoi tran dau nen thang nao thi lam cho di");
			}*/
			//=============================================================================
			if (lpInfo != 0)
			{
				...
				if (gServerInfo.m_LoaiTienNhan == 1){
					GDSetCoinSend(lpObj->Index, gServerInfo.m_TienCuocQuyetDau - tienbitru, 0, 0, ""); //
					GDSetCoinSend(lpTarget->Index, -gServerInfo.m_TienCuocQuyetDau, -0, -0, ""); // 
					gServerInfo.m_TienTrinhQuyetDau = 0; //
					//gServerInfo.m_KichHoatQuyetDau = 0; //
					if (gServerInfo.m_DuelArenaAnnounceSwitch != 0){
				...
				{
					GDSetCoinSend(lpObj->Index, 0, gServerInfo.m_TienCuocQuyetDau - tienbitru, 0, ""); //
					GDSetCoinSend(lpTarget->Index, -0, -gServerInfo.m_TienCuocQuyetDau, -0, ""); // 
					gServerInfo.m_TienTrinhQuyetDau = 0; //
					//gServerInfo.m_KichHoatQuyetDau = 0; //
					if (gServerInfo.m_DuelArenaAnnounceSwitch != 0){
				...
				{
					GDSetCoinSend(lpObj->Index, 0, 0, gServerInfo.m_TienCuocQuyetDau - tienbitru, ""); //
					GDSetCoinSend(lpTarget->Index, 0, -0, -gServerInfo.m_TienCuocQuyetDau, ""); // 
					gServerInfo.m_TienTrinhQuyetDau = 0; //
					//gServerInfo.m_KichHoatQuyetDau = 0; //
					if (gServerInfo.m_DuelArenaAnnounceSwitch != 0){
...
#endif
#if(SinhTon)
	if (lpObj->CB_Status1 != 0 || lpTarget->CB_Status1 != 0)
	{
		return; //
	}
#endif
