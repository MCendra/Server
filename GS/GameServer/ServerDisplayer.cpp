...
void CServerDisplayer::SetWindowName() // OK
{
	...
	{
		wsprintf(buff, "[%s] %s (ON: %d) %s   (License Free) %s", GAMESERVER_VERSION, gServerInfo.m_ServerName, gObjTotalUser, GAMESERVER_CLIENT, "https://www.facebook.com/hieudaica2020");
	}
...
void CServerDisplayer::PaintEventTime() // OK
{
...
#if(HONCHIENCLASS == 1)
	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "Hon Chien: ");

	if (this->ReadPKEventInfo == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->ReadPKEventInfo == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds = this->ReadPKEventInfo;
		hours = totalseconds / 3600;
		minutes = (totalseconds / 60) % 60;
		seconds = totalseconds % 60;
		wsprintf(text2, "%02d:%02d:%02d", hours, minutes, seconds);
	...
	else if (this->ReadPKEventInfo < 300)
	{
		SetTextColor(hdc, RGB(0, 190, 0));
	}
	else
...
void CServerDisplayer::PaintInvasionTime() // OK
{
	...
	for (int n = 0; n < 14; n++)
	{
		...
		SetTextColor(hdc, RGB(0, 102, 204));
		if (n == 0) { wsprintf(text1, "%d - %s", n, "Quy Xuong: "); }
		if (n == 1) { wsprintf(text1, "%d - %s", n, "Rong Do: "); }
		if (n == 2) { wsprintf(text1, "%d - %s", n, "Boss Vang: "); }
		if (n == 3) { wsprintf(text1, "%d - %s", n, "Binh Doan: "); }
		...
		if (n == 9) { wsprintf(text1, "%d - %s", n, "Sieu Boss: "); }
		if (n == 10){ wsprintf(text1, "%d - %s", n, "Boss Class: "); }
		if (n == 11){ wsprintf(text1, "%d - %s", n, "Erohim: "); }
		if (n == 12){ wsprintf(text1, "%d - %s", n, "KunDun"); }
		if (n == 13){ wsprintf(text1, "%d - %s", n, "Selupan: "); }
