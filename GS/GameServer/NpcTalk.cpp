...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj)
{
...
#if(EVENT_END_LESS)
	//=======Event End Less Tower
	if (gBEventEndLess.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
#endif
	...
	if (lpNpc->Class == gServerInfo.BAOTIEU_NPC &&
		lpNpc->X == gServerInfo.BAOTIEU_NPCX &&
		lpNpc->Y == gServerInfo.BAOTIEU_NPCY)
	{
		if (lpObj->ChonPheHanhTau == 2)
		{
			//gObjMoveGate(lpObj->Index, gServerInfo.BAOTIEU_GATE);
			OpenNPCMoveMapBaoTieu(lpObj->Index, 2);
		}
		else
		{
			GCChatTargetSend(lpObj, lpNpc->Index, gServerInfo.EventBD_Mess[0]);
		}
		return 1;
	}
	if (lpNpc->Class == gServerInfo.DAOTAC_NPC &&
		lpNpc->X == gServerInfo.DAOTAC_NPCX &&
		lpNpc->Y == gServerInfo.DAOTAC_NPCY)
	{
		if (lpObj->ChonPheHanhTau == 1)
		{
			//gObjMoveGate(lpObj->Index, gServerInfo.DAOTAC_GATE);
			OpenNPCMoveMapBaoTieu(lpObj->Index, 1);
		}
		else
		{
			GCChatTargetSend(lpObj, lpNpc->Index, gServerInfo.EventBD_Mess[1]);
		}
		return 1;
	}
	if (lpNpc->Class == gServerInfo.CHUNGBD_NPC &&
		lpNpc->X == gServerInfo.CHUNGBD_NPCX &&
		lpNpc->Y == gServerInfo.CHUNGBD_NPCY)
	{
		if (lpObj->ChonPheHanhTau >= 1 && lpObj->ChonPheHanhTau <=2)
		{
			//gObjMoveGate(lpObj->Index, gServerInfo.CHUNGBD_GATE);
			OpenNPCMoveMapBaoTieu(lpObj->Index, 3);
		}
		else
		{
			GCChatTargetSend(lpObj, lpNpc->Index, gServerInfo.EventBD_Mess[2]);
		}
		return 1;
	}
#endif
