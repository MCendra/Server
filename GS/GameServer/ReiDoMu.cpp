		...
		if (this->WinnerNumber >= 0)
		{
			...
			if (this->m_RemainTimeWinner == 28)
			{
				...
				if (lpGuildInfo == 0)
				{
					...
					LogAdd(LOG_EVENT, "[Chiếm Thành] Guild Winner: %s", lpGuildInfo->Name);
...
void CReiDoMU::SetState_START()
{
	...
	LogAdd(LOG_EVENT, "[Chiếm Thành] Bắt Đầu");
...
void CReiDoMU::StartKing()
{
	...
	LogAdd(LOG_EVENT, "[Chiếm Thành Bắt Đầu] At %2d:%2d:00", hour, minute);
