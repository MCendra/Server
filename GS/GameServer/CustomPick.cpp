...
bool CCustomPick::CommandPick(LPOBJ lpObj, char* arg)
{
	...
	if (strcmp(mode, "exc") == 0)
	{
		if (lpObj->PickupExc == 1)
		{
			...
			gLog.Output(LOG_COMMAND, "[CustomPick] Account:[%s] Name:[%s] - (Disable: %s)", lpObj->Account, lpObj->Name, mode);
			return 0;
		}
		else
		{
			...
			gLog.Output(LOG_COMMAND, "[CustomPick] Account:[%s] Name:[%s] - (Enable: %s)", lpObj->Account, lpObj->Name, mode);
			return 1;
		}
	}
