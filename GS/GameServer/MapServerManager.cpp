...
short CMapServerManager::CheckMapServerMove(int aIndex,int map,short ServerCode)
{
	...
	if(MAP_RANGE(map) == 0)
	{
		LogAdd(LOG_RED, "[MapServerMng] CheckMoveMapSvr() - Map Index doesn't exist  Account:[%s] Name:[%s] : %d",
	...
	if ( lpMapSvrData == NULL )
	{
		LogAdd(LOG_RED, "[MapServerMng] CheckMoveMapSvr() - m_lpThisMapSvrData == NULL  Account:[%s] Name:[%s] : %d",
			gObj[aIndex].Account, gObj[aIndex].Name, map);
	...
	if ( lpMapSvrData->m_used == FALSE )
	{
		LogAdd(LOG_RED, "[MapServerMng] CheckMoveMapSvr() - lpMapSvrData->m_bIN_USE == FALSE  Account:[%s] Name:[%s] : %d",
			gObj[aIndex].Account, gObj[aIndex].Name, map);
	...
	switch ( sMAP_MOVE_INFO )
	{
		...
		default:
			...
			{
				LogAdd(LOG_RED, "[MapServerMng] CheckMoveMapSvr() - Unknown MapMove Info  Account:[%s] Name:[%s] : MAP-%d / INFO-%d",gObj[aIndex].Account, gObj[aIndex].Name, map, sMAP_MOVE_INFO);
