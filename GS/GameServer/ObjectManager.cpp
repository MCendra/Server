...
#if(EVENT_END_LESS)
	//===Event End Less
	else if (gBEventEndLess.CheckUserInEvent(lpObj->Index) == 1)
	{
		gate = 0;
		map = lpObj->Map;
		x = lpObj->X;
...
#endif
		gBBuffPhe.GetOptionCuongHoa(lpObj, 1);
...
#endif
	gBBuffPhe.GetOptionCuongHoa(lpObj, 0);
	//-----------------------
#if(TULUYEN_NEW)
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex)
{
	...
	lpObj->DHSatThuong = lpMsg->DHSatThuong;
	lpObj->DHSatThuongX2 = lpMsg->DHSatThuongX2;
	lpObj->DHPhongThu = lpMsg->DHPhongThu;
	lpObj->DHHP = lpMsg->DHHP;
	lpObj->DHSD = lpMsg->DHSD;
	lpObj->DHGST = lpMsg->DHGST;
#endif

	lpObj->UserSkinPick = lpMsg->mUserSkinPick; //1 Dao Tac, 2 Bao Tieu
#if(CHONPHEDOILAP)
	lpObj->ChonPheHanhTau = lpMsg->ChonPheHanhTau;
#endif
#if(B_HON_HOAN)
	lpObj->CapDoHonHoan = lpMsg->CapDoHonHoan;
#endif
	lpObj->PointUsePhe = lpMsg->PointUsePhe;
	lpObj->LvCuongHoaHienTai = (int)gBBuffPhe.GetCapCHInPoint(lpObj->PointUsePhe);
	//LogAdd(LOG_RED, "Point User %d ", lpObj->PointUsePhe);
...
#endif
#if(EVENT_END_LESS)
	lpObj->mLuotDiEndLess = lpMsg->mLuotDiEndLess;
#endif	
		...
		if(gObj[SummonIndex].Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
		{
			...
			if (g_Survivor.ActiveEvent > -1 && g_Survivor.State == SURVIVOR_STATE_BATTLE)
			{
				if (g_Survivor.IsInSurvivorArea(lpObj) && g_Survivor.IsInSurvivorArea(lpTarget))
				{
					...
					LogAdd(LOG_BLUE, "[PKEVENT] [%s] killed [%s] | Kill Count: %d", lpObj->Name, lpTarget->Name, lpObj->SurvivorKillCount);
