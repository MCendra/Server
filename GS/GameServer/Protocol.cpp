...
void ProtocolCore(BYTE head, BYTE* lpMsg, int size, int aIndex, int encrypt, int serial)
{
	...
	LPOBJ lpObj = &gObj[aIndex]; // add core
	...
	case 0xD3:
		...
		case 0x05:
		{
			XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
			if (!mlpMsg->ThaoTac) { return; }
			gBBuffPhe.SendInfoClient(aIndex);
		}
		break;
		case 0x06:
		{
			XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
			int Point = mlpMsg->ThaoTac;
			if (!Point) { return; }
			gBBuffPhe.CongHienBuffPhe(aIndex, Point);
		}
		break;
		...
		{
			XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
			int mChonPhe = mlpMsg->ThaoTac;
			if (!mChonPhe || mChonPhe > 2) { return; }
			lpObj->ChonPheHanhTau = (BYTE)(mChonPhe);
		...
		{
			XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
			int mTypeMove = mlpMsg->ThaoTac;
			if (!mTypeMove || mTypeMove > 3) { return; }
			switch (mTypeMove)
			{
			case 1:
				gObjMoveGate(lpObj->Index, gServerInfo.DAOTAC_GATE);
				break;
			case 2:
				gObjMoveGate(lpObj->Index, gServerInfo.BAOTIEU_GATE);
				break;
			case 3:
				gObjMoveGate(lpObj->Index, gServerInfo.CHUNGBD_GATE);
				break;
			default:
				break;
			}
		...
		case 0x30:
		{
			gBEventEndLess.CGOpenWindowNPC((XULY_CGPACKET*)lpMsg, aIndex);
		}
		break;
#endif
		...
		case 0x4A:
		{
			XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
			int mTypeMove = mlpMsg->ThaoTac;
			if (!mTypeMove || mTypeMove < 1000) { return; }
			gObjMoveGate(lpObj->Index, mTypeMove);
		}
		break;
#endif
		}
	...
	case 0xFF:
		...
		case 0x06:
			...
			//==THan Ma Chien
		case 0x92:
			gThanMaChien.CGOpenWindowNPC((XULY_CGPACKET*)lpMsg, aIndex);
			break;
#endif
...
#endif
#if(CHONPHEDOILAP)
bool CheckChonPhe(int AIndex, int BIndex) //
{
	if (gServerInfo.MoChucNangChonPhe == 0 || gObj[BIndex].Type != OBJECT_USER) return 1; //OK Tiep Tuc

	if (gObj[AIndex].ChonPheHanhTau == 0)
	{
		gNotice.GCNoticeSend(AIndex, 1, 0, 0, 0, 0, 0, "Ban can gia nhap vao 1 Phe truoc !!");
		return 0;
	}
	if (gObj[AIndex].ChonPheHanhTau != gObj[BIndex].ChonPheHanhTau)
	{
		gNotice.GCNoticeSend(AIndex, 1, 0, 0, 0, 0, 0, "Khac phe khong the thuc hien thao tac nay !");
		return 0;
	}
	return 1;
}
...
void CGMoveRecv(PMSG_MOVE_RECV* lpMsg, int aIndex)
{
	...
	{
#if(CHONPHEDOILAP)
		if (lpObj->ChonPheHanhTau == 0 && gServerInfo.MoChucNangChonPhe == 1)
		{
			//OPen Cua So Chon Phe
			XULY_CGPACKET pMsg;
...
#if(MOVETAP == 1)
	if (lpObj->IsAutoMoveRunning == 0 && lpObj->Type == OBJECT_USER)
	{
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		if (lpObj->VpPlayer[n].type == OBJECT_USER)
		{
			...
			for (size_t i = 0; i < 2; i++)
			{
				...
				//-----
				if (lpTarget->Inventory[i].IsItem() && lpTarget->Inventory[i].m_Index == GET_ITEM(0, 61))
				{
					bHas = true;
					break;
				}
				...
				if (lpTarget->Inventory[i].IsItem() && lpTarget->Inventory[i].m_Index == GET_ITEM(3, 27))
				{
					bHas = true;
					break;
				}
...
#if(EVENT_MAPBAOTIEU)
void OpenNPCMoveMapBaoTieu(int aIndex,int Type)
{
	//OPen Cua So Chon Phe
	XULY_CGPACKET pMsg;
