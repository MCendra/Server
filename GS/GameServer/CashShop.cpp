	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		if (gServerInfo.m_CashShopCoinsDelay >= 1)
		{
			if ((GetTickCount() - lpObj->CashShopGoblinPointTime) >= ((DWORD)gServerInfo.m_CashShopCoinsDelay * 60000))
			{
				...
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(570), gServerInfo.m_CashShopWCoinCValue[lpObj->AccountLevel]);
				}
				if (gServerInfo.m_CashShopWCoinPValue[lpObj->AccountLevel] >= 1)
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(571), gServerInfo.m_CashShopWCoinPValue[lpObj->AccountLevel]);
				}
				if (gServerInfo.m_CashShopGoblinPointValue[lpObj->AccountLevel] >= 1)
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(572), gServerInfo.m_CashShopGoblinPointValue[lpObj->AccountLevel]);
				}
				if (gServerInfo.m_CashShopRuudValue[lpObj->AccountLevel] >= 1)
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(573), gServerInfo.m_CashShopRuudValue[lpObj->AccountLevel]);
				}
			}
		}
		//--
