...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo)
{
	...
	if (lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_MONSTER)
	{
		if (combo == 0 && skill != SKILL_NONE)
		{
			...
			{
				gLog.Output(LOG_HACK, "[HackAutoSkill] Account:[%s] Name:[%s] (%d)", lpObj->Account, lpObj->Name, skill);
				return 0;
			}
		...
		if (combo == 0 && skill == SKILL_NONE)
		{
			if (lpTarget->SummonIndex > 1)
			{
				gLog.Output(LOG_HACK, "[HackAutoSkill] Account:[%s] Name:(%d)", lpObj->Account, lpObj->Name, skill);
				return 0;
			}
		}
	}
