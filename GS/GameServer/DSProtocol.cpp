	...
	switch(head)
	{
		...
		case 0xD9:
			...
			case 0x02:
				...
				break;
			case 0x03:
				gBBuffPhe.CacheDataBuffPhe((BUFFPHE_REQUESTDS_SETINFO*)lpMsg);
				break;
			case 0x04:
				gBBuffPhe.RecvCharInfoAddBot((INFOCHAR_BUFFPHE*)lpMsg);
				break;
			}
			break;
#endif
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg)
{
	...
	gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessage.GetMessage(990), gCustomGHRS.maxResets);
	...
	else
	{
		if (gServerInfo.m_OnlineUserSwitch == 1)
		{
			...
			if (gServerInfo.m_ThongBaoOnline == 2)
			{
				...
				{
					wsprintf(szTemp1, "[Đạo Tặc] %s đã tham chiến ", lpObj->Name);
				}
				else if (lpObj->ChonPheHanhTau == 2)
				{
					wsprintf(szTemp1, "[Bảo Tiêu] %s đã tham chiến ", lpObj->Name);
				} else
	...
	//======================================================================================
	gLog.Output(LOG_KET_NOI, "[Obj] [%d] Name (%s)", lpObj->Index, lpObj->Name);
...
void GDCharacterInfoSaveSend(int aIndex)
{
	...
	pMsg.DHSatThuong = lpObj->DHSatThuong;
	pMsg.DHSatThuongX2 = lpObj->DHSatThuongX2;
	pMsg.DHPhongThu = lpObj->DHPhongThu;
	pMsg.DHHP = lpObj->DHHP;
	pMsg.DHSD = lpObj->DHSD;
	pMsg.DHGST = lpObj->DHGST;
#endif
...
#if(EVENT_END_LESS)
	pMsg.mLuotDiEndLess = lpObj->mLuotDiEndLess;
#endif
...
#if(CHONPHEDOILAP)
	pMsg.ChonPheHanhTau = lpObj->ChonPheHanhTau; //1 Dao Tac, 2 Bao Tieu
#endif
#if(B_HON_HOAN)
	pMsg.CapDoHonHoan = lpObj->CapDoHonHoan; 
#endif
	pMsg.PointUsePhe = lpObj->PointUsePhe;
	gDataServerConnection.DataSend((BYTE*)&pMsg,sizeof(pMsg));
...
void GDSetCoinSend(int aIndex, int value1, int value2, int value3, char * LogName)
{
    ...
    gDataServerConnection.DataSend((BYTE*)&pMsg,pMsg.header.size);
		...
		LogAdd(LOG_ALERT,"Account:[%s] Name:[%s] WZ_SetCoin(%s) WCoinC: %d, WCoinP: %d, WCoinG: %d",lpObj->Account,lpObj->Name,LogName,value1,value2,value3);
	
		gLog.Output(LOG_TIEN_TE, "Account:[%s] Name:[%s] WZ_SetCoin(%s) WCoinC: %d, WCoinP: %d, WCoinG: %d", lpObj->Account, lpObj->Name, LogName, value1, value2, value3);

		gLog.Output(LOG_TIEN_TE, "Account:[%s] Name:[%s] (WCoinC Hiện Tại: %d) (WCoinP Hiện Tại: %d) (WCoinG Hiện Tại: %d)", lpObj->Account, lpObj->Name, lpObj->Coin1, lpObj->Coin2, lpObj->Coin3);
...
void GDBotInfoSend(int aIndex, int Rank, int TypeTop)
{
	...
	LogAdd(LOG_RED, "DEBUG GDBotInfoSend >> Lấy Dữ Liệu Nhân Vật Tốp %d Type %d", Rank, TypeTop);
}
#endif
