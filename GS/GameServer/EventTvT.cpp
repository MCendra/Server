...
void CTvTEvent::SetState_CLEAN()
{
	...
	if (this->ScoreBlue > this->ScoreRed)
	{
		...
		this->NoticeSendToAll(0, "Đội Xanh Đã Chiến Thắng!");
	}
	else if (this->ScoreRed > this->ScoreBlue)
	{
		TeamReward = TVT_REDTEAM;
		this->NoticeSendToAll(0, "Đội Đỏ Đã Chiến Thắng!");
	}
...
bool CTvTEvent::CheckEnterEnabled(LPOBJ lpObj)
{
	...
	// SETtIME
	lpObj->TimeTeamvsTeam = this->m_EventTimeReg;
...
void CTvTEvent::DivisionTeam()
{
	...
	for (int n = 0; n<MAX_TVTEVENT_USER; n++)
	{
		if (this->User[n].Team == 1)
		{
			...
			gNotice.GCNoticeSend(this->User[n].Index, 0, 0, 0, 0, 0, 0, "[Đội Xanh]: Giết %d Mạng [Đội Đỏ]: Giết %d Mạng", this->ScoreBlue, this->ScoreRed);
		}
		else if (this->User[n].Team == 2)
		{
			...
			gNotice.GCNoticeSend(this->User[n].Index, 0, 0, 0, 0, 0, 0, "[Đội Đỏ]: Giết %d Mạng [Đội Xanh]: Giết %d Mạng", this->ScoreRed, this->ScoreBlue);
		}
	}
}
...
void CTvTEvent::UserDieProc(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	gNotice.GCNoticeSend(lpUserA->Index, 1, 0, 0, 0, 0, 0, "Bạn Đã Bị Chết %d Lần", lpUserA->Deaths);
	gNotice.GCNoticeSend(lpUserB->Index, 1, 0, 0, 0, 0, 0, "Bạn Đã Giết %d Người", lpUserB->Kills);
	...
	for (int i = 0; i<MAX_TVTEVENT_USER; i++)
	{
		...
		{
			gNotice.GCNoticeSend(this->User[i].Index,0,0,0,0,0,0,"[Đội Xanh]: Giết %d Mạng [Đội Đỏ]: Giết %d Mạng",this->ScoreBlue,this->ScoreRed);

		}
		else if (this->User[i].Team == TVT_REDTEAM)
		{
			gNotice.GCNoticeSend(this->User[i].Index,0,0,0,0,0,0,"[Đội Đỏ]: Giết %d Mạng [Đội Xanh]: Giết %d Mạng",this->ScoreRed,this->ScoreBlue);
		}
	}
}
...
void CTvTEvent::TimeTvTEvent(int aIndex)
{
	...
	if (lpObj->TimeTeamvsTeam < 0) { lpObj->TimeTeamvsTeam = 0; }

	if (lpObj->Map != MAP_TVTEVENT)
	{
		return;
	}
	if (lpObj->TimeTeamvsTeam < 1)
	{
		return;
	}
	if (this->GetState() == TVT_EVENT_STATE_START)
	{
		if (lpObj->TimeTeamvsTeam >= 1)
		{
			lpObj->TimeTeamvsTeam = lpObj->TimeTeamvsTeam - 1;
...
void CTvTEvent::SendInfoTimeTvTEvent(int aIndex)
{
	...
	pMsg.TimeConLaiTvTEvent = lpObj->TimeTeamvsTeam;
...
void CTvTEvent::StartTvT()
{
	...
	LogAdd(LOG_EVENT, "[Sự Kiện TVT Bắt Đầu] At %2d:%2d:00", hour, minute);
