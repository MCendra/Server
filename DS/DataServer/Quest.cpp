...
void CQuest::GDQuestKillCountRecv(SDHP_QUEST_KILL_COUNT_RECV* lpMsg, int index)
{
	SDHP_QUEST_KILL_COUNT_SEND pMsg{};

	pMsg.Header.set(DS_HEAD_QUEST, DS_SUB_QUEST_KILLCOUNT_LOAD, sizeof(pMsg));
	...
	if (!gQueryManager.ExecQuery("SELECT * FROM QuestKillCount WHERE Name='%s'", lpMsg->CharacterName) || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
