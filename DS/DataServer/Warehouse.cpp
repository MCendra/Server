...
void CWarehouse::GDWarehouseGuildUpdate(SDHP_WAREHOUSE_GUILD_STATUS_UPDATE_RECV * lpMsg)// OK
{
	gQueryManager.ExecQuery("UPDATE WarehouseGuild SET StatusRender=%d WHERE Guild='%s'",lpMsg->Status,lpMsg->Name);
	gQueryManager.Close();
}

void CWarehouse::GDWarehouseGuildConsult(SDHP_WAREHOUSE_GUILD_STATUS_RECV * lpMsg, int index) // OK
{
	SDHP_WAREHOUSE_GUILD_STATUS_SEND pMsg;

	pMsg.header.set(0x05, 0x76, sizeof(pMsg));

	pMsg.index = lpMsg->index;

	if(gQueryManager.ExecQuery("SELECT StatusRender FROM WarehouseGuild WHERE Guild='%s'",lpMsg->Name) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		pMsg.Status = 0;
		gQueryManager.Close();
	}
